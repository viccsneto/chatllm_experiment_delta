...
**O que significa:** É o registro do debate técnico que encerra o ciclo da tarefa controlada pelo pipeline, comprovando se o desenvolvedor é capaz de sustentar as escolhas arquiteturais e lógicas do código que acabou de implementar.
...
**Escopo importante:** este passo pertence exclusivamente ao ciclo da **tarefa controlada pelo pipeline**. Ele deve ser solicitado logo após o preenchimento do `REACTO.md` daquela tarefa — não é uma revisão final do experimento, não é aplicado à tarefa livre, e não deve ser adiado para depois que ambas as tarefas estiverem prontas.
